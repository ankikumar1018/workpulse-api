...
After updating SQLAlchemy models, run these commands from the backend root:

```bash
docker compose up -d postgres
export DATABASE_URL="postgresql+psycopg://postgres:postgres@127.0.0.1:5432/workpulse"
```
...
```bash
# Check for model changes without creating a migration
...
# Roll back one migration when necessary
uv run alembic downgrade -1
```

If `uv` selects an environment without the backend dependencies, use the local
Alembic executable:
